  ...
  static async loadPaper(filePath) {
    try {
      ...
      const assetEntries = zip.getEntries().filter(
        (entry) => entry.entryName.startsWith("assets/") && !entry.isDirectory
      );
...
`;
          ...
          break;
        case "citation":
          if ((_c = block.attrs) == null ? void 0 : _c.refId) {
...
`;
  ...
  // 渲染PDF
  async renderPDF(typstContent, outputPath, assets = {}) {
    return new Promise((resolve, reject) => {
      ...
      const typst = child_process.spawn(this.typstPath, ["compile", tempTypstPath, outputPath]);
      let errorOutput = "";
...
async function createWindow() {
  ...
  win.on("maximize", () => win == null ? void 0 : win.webContents.send("window-maximized", true));
  win.on("unmaximize", () => win == null ? void 0 : win.webContents.send("window-maximized", false));
}
...
electron.ipcMain.handle("load-paper", async () => {
  var _a;
...
electron.ipcMain.handle("save-paper", async (_, { filePath, data, assets }) => {
...
electron.ipcMain.handle("render-preview", async (_, { data, assets }) => {
  try {
